/*
  # COMPLETE AUTO CHECKOUT SYSTEM REBUILD - HOSTINGER COMPATIBLE
  
  This SQL file completely rebuilds the auto checkout system for L.P.S.T Hotel
  to ensure 100% reliable daily 10:00 AM execution on Hostinger servers.
  
  INSTRUCTIONS:
  1. Run this ONCE in phpMyAdmin
  2. This will drop old conflicting tables and create fresh ones
  3. All flags will be reset for fresh start
  4. System will be ready for daily 10:00 AM execution
  
  FEATURES:
  - Daily 10:00 AM auto checkout (EXACT timing)
  - NO automatic payment calculation (admin marks payments manually)
  - Comprehensive logging system
  - Hostinger MySQL/MariaDB compatible
  - Foolproof duplicate prevention
...
('auto_checkout_system_version', '4.0', 'Auto checkout system version - Complete rebuild for Hostinger'),
...
('auto_checkout_force_10am_only', '1', 'Force execution only between 10:00-10:05 AM'),
...
('system', 'COMPLETE AUTO CHECKOUT REBUILD - Hostinger Compatible: Simplified system, no payment calculation, manual payment only, exact 10:00 AM execution');

-- Create verification record
INSERT INTO `system_settings` (`setting_key`, `setting_value`, `description`) VALUES
('system_rebuild_completed', NOW(), 'Complete system rebuild finished - Ready for 10:00 AM execution'),
('hostinger_import_date', NOW(), 'Date when SQL was imported in Hostinger phpMyAdmin'),
('system_status', 'READY', 'System is ready for daily 10:00 AM auto checkout');

-- Insert test cron execution log to verify table structure
INSERT INTO `cron_execution_logs` 
(`execution_date`, `execution_time`, `execution_type`, `execution_status`, `notes`, `server_time`) 
VALUES 
(CURDATE(), '10:00:00', 'test', 'success', 'System rebuild completed - Tables created successfully in Hostinger', NOW());

-- Final verification queries
SELECT 'AUTO CHECKOUT SYSTEM REBUILD COMPLETED SUCCESSFULLY' as STATUS;
SELECT COUNT(*) as ACTIVE_BOOKINGS_READY FROM `bookings` WHERE `status` IN ('BOOKED', 'PENDING') AND `auto_checkout_processed` = 0;
SELECT setting_value as AUTO_CHECKOUT_STATUS FROM `system_settings` WHERE setting_key = 'auto_checkout_enabled';
SELECT setting_value as DAILY_EXECUTION_TIME FROM `system_settings` WHERE setting_key = 'auto_checkout_time';